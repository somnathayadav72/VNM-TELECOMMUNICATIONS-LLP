...
function Hero() {
  ...
  return <section className="hero" id="top" onPointerMove={movePointer} onPointerLeave={resetPointer}><div className="hero__grid" aria-hidden="true" /><div className="hero__halo" aria-hidden="true" /><div className="wrap hero__inner"><motion.div className="hero__copy" style={{ opacity: copyOpacity, y: copyY }}><p className="eyebrow">B2B MOBILE WHOLESALE · SINCE {site.founded}</p><h1><span className="hero__line hero__line--one">VERIFIED</span><span className="hero__line hero__line--two">DEVICES.</span><span className="hero__line hero__line--three">READY FOR</span><span className="hero__line hero__line--four">RESALE.</span></h1><p className="hero__body">New and certified refurbished smartphones, tablets, wearables and accessories—graded, documented and shipped to resellers worldwide.</p><div className="hero__actions"><a className="button button--dark" href="#categories">Browse stock categories <ArrowUpRight size={16} /></a><a className="text-link" href="#contact">Request a quote <ArrowUpRight size={16} /></a></div></motion.div><motion.div className="hero__visual" style={{ scale: visualScale, y: visualY }}><span className="hero__serial">VNM / 1999—2026 / 001</span><motion.div className="hero__device" style={{ x: pointerX, y: pointerY }}><AssetImage src="/images/devices/hero-device-stack.webp" fallback="/generated/devices/hero-device-stack.svg" alt="Fictional stack of smartphones, tablet, laptop and wearable devices" priority sizes="(max-width: 768px) 95vw, 58vw" /></motion.div><div className="hero__measure"><span>01</span><i /><span>04</span></div><span className="hero__orbit hero__orbit--one" /><span className="hero__orbit hero__orbit--two" /></motion.div><div className="hero__summary"><span className="eyebrow">INVENTORY SNAPSHOT</span><strong>{site.annualUnits}</strong><small>units shipped / year</small></div><div className="hero__scroll"><span>Scroll to explore</span><i /></div></div></section>;
}

function TrustMarquee() {
  const rows = [{ label: "SUPPLYING ALL MAJOR DEVICE ECOSYSTEMS", items: "APPLE · SAMSUNG · GOOGLE · XIAOMI · MOTOROLA · OPPO · ONEPLUS · HUAWEI" }, { label: "GLOBAL DELIVERY NETWORK", items: "DHL · UPS · FEDEX · DPD · GLS · TNT" }];
...
function Stats() {
  const stats = [["25+", "Years in trade"], [site.countriesServed, "Countries served"], [site.annualUnits, "Units shipped / year"], [site.dispatchTime, "Average dispatch"]];
...
function CategoryGrid() {
  return <section className="section section--light categories" id="categories"><div className="wrap"><SectionIntro eyebrow="PRODUCT CATEGORIES" title="Inventory built for every sales channel." copy="A dependable mix of sell-through friendly categories, with the grading detail and documentation your buyers need." /><div className="categories__grid">{categories.map((category, index) => <Reveal key={category.id} className={`category-card-wrap category-card-wrap--${category.id}`} delay={index * 0.06}><CategoryCard category={category} index={index} /></Reveal>)}</div></div></section>;
}
...
function Services() {
  const services = [{ icon: Boxes, title: "Wholesale supply", copy: "New and refurbished smartphones, tablets, wearables and accessories supplied in scalable quantities.", color: "#3d63ff" }, { icon: BadgeCheck, title: "Testing & grading", copy: "Functional checks, battery thresholds and transparent cosmetic grades support confident purchasing.", color: "#b8f36a" }, { icon: Globe2, title: "Global logistics", copy: "Insured international dispatch with shipment documentation and trusted logistics networks.", color: "#f2c84b" }];
  return <section className="section services" id="company"><div className="wrap"><SectionIntro eyebrow="WHOLESALE SERVICES" title="One supply partner. Every critical step." /><div className="services__grid">{services.map(({ icon: Icon, ...service }, index) => <Reveal key={service.title} delay={index * 0.08}><article className="service-card" style={{ "--service-color": service.color }}><div className="service-card__icon"><Icon size={22} /></div><p className="eyebrow">0{index + 1}</p><h3>{service.title}</h3><p>{service.copy}</p><span className="service-card__line" /></article></Reveal>)}</div></div></section>;
}

function Process() {
  const steps = [["Request", "Tell us the models, grades and quantities you need."], ["Quote", "Receive clear per-unit pricing and stock conditions."], ["Confirm", "Approve the order, documentation and payment terms."], ["Dispatch", "Receive insured shipment tracking and export documents."]];
  return <section className="section process"><div className="wrap"><SectionIntro eyebrow="PROCESS" title="From stock request to tracked shipment." /><div className="process__rail">{steps.map(([title, copy], index) => <Reveal className="process-step" key={title} delay={index * 0.08}><span className="process-step__number">0{index + 1}</span><div className="process-step__dot" /><h3>{title}</h3><p>{copy}</p>{index < steps.length - 1 && <i className="process-step__connector" />}</Reveal>)}</div><div className="process__stamp"><Package size={17} /> ORDER FLOW / 01—04 / CONTROLLED DISPATCH</div></div></section>;
}
